...
import { addHours, isWithinInterval, isBefore, subMinutes, differenceInMinutes, intervalToDuration, isAfter } from 'date-fns';
...
export const getMeetingStatus = (interview: Interview) => {
  ...
  const shortlyStartingTime = subMinutes(interviewStartTime, 5);
  ...
  if (isWithinInterval(now, { start: shortlyStartingTime, end: interviewStartTime })) {
    const minutesLeft = differenceInMinutes(interviewStartTime, now);
