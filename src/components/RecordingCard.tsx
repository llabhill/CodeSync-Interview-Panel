...
//npm i date-fns:- for formating the date
function RecordingCard({recording}:{recording:CallRecording}) {
    const handleCopyLink=async()=>{
        try {
          ...
          toast.error("Failed to copy Recording url to clipboard ")  
        }
    };


    const formattedStartTime=recording.start_time
    ? format(new Date(recording.start_time),"MMM d, yyyy, hh:mm a"):"Unknown";

    const duration=recording.start_time&&recording.end_time?
    calculateRecordingDuration(recording.start_time,recording.end_time):"Unknown Duration"
