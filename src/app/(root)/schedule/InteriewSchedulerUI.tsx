...
function InteriewSchedulerUI() {
  ...
  //state taht check form(dialogue) part ism open or not
  const [open, setOpen] = useState(false)

  //state checked is interview creating or not
  ...
  const scheduleMeeting = async () => {
    ...
    if (!formData.candidateId || formData.interviewerIds.length === 0) {
      toast.error("Please select both Candidate and at atlaest one Interviewer");
      return
    }
    setIsCreating(true);
    ...
    try {
      ...
      //create entry pf interview in db
...
export default InteriewSchedulerUI
