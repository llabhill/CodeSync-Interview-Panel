...
function DashboardPage() {
  ...
  const updatedStatus=useMutation(api.interviews.updateInterviewStatus);

  //calling handstatus fucntion weather interview is failed or succeeded

  const handleStatusUpdate=async(interviewId:Id<"interviews">,status:string)=>{
    try {
      await updatedStatus({id:interviewId,status})
  ...
  //now we want to place interviews acc. to its status
            ...
            <section key={category.id}>
              ...
              <div className="flex items-center gap-2 mb-4">
                  <h2 className="text-xl font-semibold">{category.title}</h2>
                    {/* displaying number of interviews */}
